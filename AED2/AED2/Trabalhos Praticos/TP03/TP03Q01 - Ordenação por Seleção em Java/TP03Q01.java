...
class TP03Q01 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < numEntrada; i++) {
            lista.inserirFim(map.get(Integer.valueOf(entrada[i])));
        }
        lista.selecaoNome();
...
class Lista {
    ...
    public void selecaoNome() {
        for (int i = 0; i < (array.length - 1); i++) {
            String menor = array[i].getName();
            int m = i;
            for (int j = (i + 1); j < array.length; j++) {
                int index = 0;
                if (array[j].getName().charAt(index) < menor.charAt(index)) {
                    menor = array[j].getName();
                    m = j;
                    //System.out.println(array[i] + " ----- " + array[j] + " " + j);
                } else if (array[j].getName().charAt(index) == menor.charAt(index)) {
                    //System.out.println(array[j].getName() + " ----- " + menor);
                    index++;
                    if (index != -1) {
                        do {
                            //System.out.println(array[j].getName().charAt(index) + " < " + menor.charAt(index));
                            if (array[j].getName().charAt(index) < menor.charAt(index)) {
                                menor = array[j].getName();
                                m = j;
            ...
            array[i] = array[m];
            array[m] = temp;
            //System.out.println(i + ": " + array[i] + " " + m + ": " + array[m]);
        }
    }
}