...
class TP03Q06 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < numEntrada; i++) {
            array[i] = map.get(Integer.valueOf(entrada[i]));
        }
        quicksort(0, array.length-1, array);
        for (int i = 0; i < array.length; i++) {
            Game game = array[i];
            MyIO.println(game.getApp_ID() + " " + game.getName() + " " + toString(game.getData()) + " " + game.getOwners() + " " + 
    ...
    private static void quicksort(int esq, int dir, Game[] array) {
        int i = esq, j = dir;
        Game pivo = array[(dir+esq)/2];
        while (i <= j) {
            while (array[i].getData().before(pivo.getData())) i++;
            while (array[j].getData().after(pivo.getData())) j--;
            if (i <= j) {
                swap(i, j, array);
                i++;
                j--;
