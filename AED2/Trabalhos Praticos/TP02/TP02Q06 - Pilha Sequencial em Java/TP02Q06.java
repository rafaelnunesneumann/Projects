    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < numEntrada2; i++) {
            if (entrada2[i].startsWith("I")) {
                ...
                pilha.inserir(map.get(Integer.valueOf(separado[1])));
            }
        }
        ...
        for (int i = 0; i < numEntrada2; i++) {
            if (entrada2[i].startsWith("R")) {
                Game game = pilha.remover();
                System.out.println("(R) " + game.getName());
            }
        }
        for (int i = 0; i < pilha.getAll().size(); i++) {
            Game game = pilha.getAll().get(pilha.getAll().size() - 1 - i);
...
class Pilha {
	...
	public ArrayList<Game> getAll() throws Exception {
        ArrayList<Game> array = new ArrayList<>();
		for (Celula i = topo; i != null; i = i.prox) {
            Game game = i.elemento;
            array.add(game);
		}
        return array;
	}
